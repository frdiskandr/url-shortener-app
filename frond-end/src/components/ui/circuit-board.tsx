        ...
        return (
          ...
          >
            ...
            <motion.div
              ...
              style={{ backgroundColor: node.highlightColor || statusColor }}
              animate={
            ...
            <div
              ...
              style={{ borderColor: node.highlightColor || statusColor }}
            />
            ...
            {node.status === "active" && (
              <motion.div
                className="absolute inset-0 rounded-lg"
                style={{
                  boxShadow: `0 0 20px ${(node.highlightColor || statusColor)}40, inset 0 0 10px ${(node.highlightColor || statusColor)}20`,
                }}
            ...
            <div className="relative z-10 flex flex-col items-center justify-center">
              {node.icon && (
                <div style={{ color: node.textColor || statusColor }}>{node.icon}</div>
              )}
            </div>
            ...
            {node.label && (
              <div
                ...
                style={{ color: node.textColor || statusColor }}
              >
                {node.label}
              </div>
            )}
...
interface CircuitPatternProps extends Omit<CircuitBoardProps, "nodes" | "connections"> {
  pattern: "data-flow" | "network" | "processor" | "tree" | "url-flow"
}
...
function CircuitPattern({ pattern, ...props }: CircuitPatternProps) {
  const patterns = {
    ...
    "url-flow": {
      nodes: [
        { id: "client", x: 100, y: 180, label: "Client", status: "active" as const, size: "sm" as const },
        { id: "frondend", x: 280, y: 180, label: "Frond-End", status: "processing" as const, size: "md" as const },
        { id: "backend", x: 460, y: 180, label: "Backend", status: "active" as const, size: "md" as const },
        { id: "postgres", x: 600, y: 120, label: "Postgres", status: "active" as const, size: "sm" as const },
        { id: "redis", x: 600, y: 240, label: "Redis", status: "processing" as const, size: "sm" as const, highlightColor: "rgba(249, 115, 22, 0.92)", textColor: "rgba(255, 255, 255, 0.95)" },
        { id: "redirect", x: 780, y: 180, label: "Redirect", status: "active" as const, size: "sm" as const },
      ],
      connections: [
        { from: "client", to: "frondend", animated: true },
        { from: "frondend", to: "backend", animated: true },
        { from: "backend", to: "postgres", animated: true },
        { from: "backend", to: "redis", animated: true },
        { from: "frondend", to: "redirect", animated: true, color: "rgba(148, 163, 184, 0.8)", pulseColor: "rgba(148, 163, 184, 0.95)" },
      ],
    },
    processor: {
      nodes: [
